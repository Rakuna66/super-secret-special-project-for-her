...
**Nothing may outlive its use.** The playlist durations come from a throwaway `Audio` element per track; each one is released (`removeAttribute('src')` then `load()`) as soon as its `loadedmetadata` fires, with an `error` listener so a failed probe doesn't hang on either. Without that the page keeps a live media element per song for its whole life. The same instinct applies to `drawViz`, which returns early when the player view isn't active rather than repainting a canvas nobody can see, and to the two "stuck flag" guards — `analysisBusy` and `priming` — which would each silently disable a feature for the rest of the session if their one clearing path were ever missed.
...
**Visualizer.** `resizeCanvas` must keep its `if (!canvas.offsetWidth) return` guard. The canvas lives inside `#viewPlayer`, which is `display: none` while the card tab is up, and hidden elements measure zero — so a resize on that tab used to set the bitmap to 0×0 and leave the visualizer permanently blank after switching back. Rotating the phone on the card view was enough to trigger it. `switchTab` re-measures on the way back in for the same reason.
...
**Layout stability is deliberate.** Mobile browser chrome showing/hiding changes `vh`, which visibly reflowed the UI. Three places now pin dimensions explicitly instead of letting flex/viewport units decide: the shell (`width: min(460px, 100vw)`, `height: 100dvh`), the photo panel (`syncPhotoPanelHeight` sets an explicit pixel height, re-run on resize), and the playlist (`lockPlaylistHeight` writes an explicit pixel height once at startup, sized from the space actually left over and snapped down to half rows). Replacing any of these with `flex: 1` or `vh`-based sizing reintroduces the jitter.

The playlist deliberately measures twice and then stops: once inline, and again on `document.fonts.ready`, because row height comes out a couple of pixels large while the fallback face is still in use. That second pass lands while the boot screen is still covering the app, so nothing visibly resizes. It must not be wired to `resize` — that is exactly the browser-chrome jitter the fixed height exists to prevent.

**Text must stay pinned.** `html, body` sets `text-size-adjust: 100%`. Without it, iOS inflates text inside wide overflowing blocks ("font boosting"), so a title or why-note long enough to marquee rendered visibly larger than one that fits — the same authored 9px reading as two different sizes as the track changed. It does not reproduce on desktop, where every song measures identically at 9px, so this is only ever visible on a phone.
